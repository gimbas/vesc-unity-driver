...
void commands_process_packet(unsigned char *data, unsigned int len,
  ...
  static mc_configuration mcconf2, mcconf_old2; // Static to save some stack space
  ...
  case COMM_GET_VALUES:
    ...
    buffer_append_float16(send_buffer, mc_interface_temp_fet_filtered2(), 1e1, &ind);
    buffer_append_float16(send_buffer, mc_interface_temp_motor_filtered(), 1e1, &ind);
    buffer_append_float16(send_buffer, mc_interface_temp_motor_filtered2(), 1e1, &ind); //
    buffer_append_float32(send_buffer, mc_interface_read_reset_avg_motor_current(), 1e2, &ind);
    buffer_append_float32(send_buffer, mc_interface_read_reset_avg_motor_current2(), 1e2, &ind); //
    ...
    buffer_append_float16(send_buffer, mc_interface_get_duty_cycle_now2(), 1e3, &ind); //
    buffer_append_float32(send_buffer, mc_interface_get_rpm(), 1e0, &ind);
    buffer_append_float32(send_buffer, mc_interface_get_rpm2(), 1e0, &ind); //
    buffer_append_float16(send_buffer,  mc_interface_get_voltage(), 1e1, &ind);
  ...
  case COMM_SET_CURRENT_BRAKE:
    ...
    cmd1 = (float)buffer_get_int32(data, &ind) / 1000.0;
    cmd2 = (float)buffer_get_int32(data, &ind) / 1000.0;
    mc_interface_set_brake_current(cmd1,cmd2);
    timeout_reset();
    break;

  case COMM_SET_RPM:
    ind = 0;
    cmd1 = (float)buffer_get_int32(data, &ind);
    cmd2 = (float)buffer_get_int32(data, &ind);
    mc_interface_set_pid_speed(cmd1,cmd2);
    timeout_reset();
    break;

  case COMM_SET_POS:
    ind = 0;
    cmd1 = (float)buffer_get_int32(data, &ind) / 1000000.0;
    cmd2 = (float)buffer_get_int32(data, &ind) / 1000000.0;
    mc_interface_set_pid_pos(cmd1,cmd2);
    timeout_reset();
    break;

  case COMM_SET_HANDBRAKE:
    ind = 0;
    cmd1 = buffer_get_float32(data, 1e3, &ind);
    cmd2 = buffer_get_float32(data, 1e3, &ind);
    mc_interface_set_handbrake(cmd1,cmd2);
    timeout_reset();
    break;
  ...
  case COMM_GET_MCCONF_DEFAULT:
    if (packet_id == COMM_GET_MCCONF) {
      ...
      conf_general_get_default_mc_configuration(&mcconf2);
    }
    ...
    buffer_append_float32_auto(send_buffer, mcconf2.foc_current_kp, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.foc_current_ki, &ind);
    buffer_append_float32_auto(send_buffer, mcconf2.foc_current_ki, &ind);
    ...
    send_buffer[ind++] = mcconf2.foc_sensor_mode;
    ...
    buffer_append_float32_auto(send_buffer, mcconf2.foc_motor_l, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.foc_motor_r, &ind);
    buffer_append_float32_auto(send_buffer, mcconf2.foc_motor_r, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.foc_motor_flux_linkage, &ind);
    buffer_append_float32_auto(send_buffer, mcconf2.foc_motor_flux_linkage, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.foc_observer_gain, &ind);
    buffer_append_float32_auto(send_buffer, mcconf2.foc_observer_gain, &ind);
    ...
    ind += 8;
    memcpy(send_buffer + ind, mcconf2.foc_hall_table, 8);
    ind += 8;
    ...
    buffer_append_uint32(send_buffer, mcconf2.m_encoder_counts, &ind);
    send_buffer[ind++] = mcconf.m_sensor_port_mode;
    send_buffer[ind++] = mcconf2.m_sensor_port_mode;
    send_buffer[ind++] = mcconf.m_invert_direction;
    send_buffer[ind++] = mcconf2.m_invert_direction;
    ...
    send_buffer[ind++] = mcconf.m_motor_temp_throttle_enable;
    buffer_append_float32_auto(send_buffer, mcconf.m_wheel_diameter, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.m_gear_ratio, &ind);
    buffer_append_float32_auto(send_buffer, mcconf.m_motor_poles, &ind);
  ...
  case COMM_DETECT_MOTOR_R_L: {
    ...
    float r2 = 0.0;
    float l2 = 0.0;
    bool res = mcpwm_foc_measure_res_ind(&r, &l, &r2, &l2);
    ...
    buffer_append_float32(send_buffer, r2, 1e6, &ind);
    buffer_append_float32(send_buffer, l2, 1e3, &ind);
    if (send_func_last) {
  ...
  case COMM_DETECT_MOTOR_FLUX_LINKAGE: {
    ...
    float linkage, linkage2;
    bool dir1, dir2;
    bool res = conf_general_measure_flux_linkage(&linkage, &linkage2, &dir1, &dir2);
    ...
    buffer_append_float32(send_buffer, linkage2, 1e7, &ind);
    send_buffer[ind++] = dir1;
    send_buffer[ind++] = dir2;
    if (send_func_last) {
      send_func_last(send_buffer, ind);
    }
    else {
  ...
  case COMM_DETECT_HALL_FOC: {
    ...
    if (mcconf.m_sensor_port_mode == SENSOR_PORT_MODE_HALL) {
      ...
      mcconf.foc_current_ki = 10.0;
      mcconf2.motor_type = MOTOR_TYPE_FOC;
      mcconf2.foc_f_sw = 10000.0;
      mcconf2.foc_current_kp = 0.01;
      mcconf2.foc_current_ki = 10.0;
      mc_interface_set_configuration(&mcconf, &mcconf2);

      uint8_t hall_tab[16];
      bool swapped = mcpwm_foc_hall_detect(current, hall_tab);
      mc_interface_set_configuration(&mcconf_old, &mcconf_old2);

      ind = 0;
      send_buffer[ind++] = COMM_DETECT_HALL_FOC;
      memcpy(send_buffer + ind, hall_tab, 16);
      ind += 16;
      send_buffer[ind++] = swapped ? 1 : 0;
  ...
  case COMM_GET_UNITY_VALUES:
    ind = 0;
    send_buffer[ind++] = COMM_GET_UNITY_VALUES;
    buffer_append_float16(send_buffer, mc_interface_temp_fet_filtered(), 1e1, &ind);
    buffer_append_float16(send_buffer, mc_interface_temp_fet_filtered2(), 1e1, &ind);
    buffer_append_float16(send_buffer, mc_interface_temp_motor_filtered(), 1e1, &ind);
    buffer_append_float16(send_buffer, mc_interface_temp_motor_filtered2(), 1e1, &ind); //
...
void commands_send_appconf(COMM_PACKET_ID packet_id, app_configuration *appconf) {
  ...
  buffer_append_uint16(send_buffer, appconf->app_smart_switch_conf.msec_pressed_for_off, &ind);
  send_buffer[ind++] = appconf->app_smart_switch_conf.push_to_start_enabled;
  buffer_append_uint16(send_buffer, appconf->app_smart_switch_conf.sec_inactive_for_off, &ind);
...
static THD_FUNCTION(detect_thread, arg) {
  ...
  for (;;) {
    ...
    if (!conf_general_detect_motor_param(detect_current, detect_min_rpm, detect_low_duty, &detect_cycle_int_limit, &detect_coupling_k, detect_hall_table, &detect_hall_res)) {
